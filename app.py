...
def analyze_real_estate_data(suumo_file, rules_file):
    """
    アップロードされたSUUMO(Excel/CSV)とrules(CSV)から、
    1. 各間取りのベース単価
    2. 設備や条件による加減算ルール
    を算出する。
    """
    ...
    # 6. 間取りのグルーピング（正規表現で全角半角・表記ゆれを吸収）
    madori_col = '間取り' if '間取り' in df_suumo.columns else '間取' if '間取' in df_suumo.columns else None
    
    def map_madori(m):
        m = str(m).upper().replace(' ', '').replace('　', '') # 全角半角スペース除去、大文字化
        import unicodedata
        m = unicodedata.normalize('NFKC', m) # 全角英数を半角に
    ...
    for madori in madori_list:
        ...
        # ⚠️ 修正ポイント：1件でもあれば強引にモデルを作る（無い場合のみスキップ）
        if len(df_m) < 1:
        ...
        base_tanka_dict[madori] = tanka_series.median() if not tanka_series.empty else 4000
        
        if not tanka_series.empty:
            market_stats[madori] = {
        ...
        # データが少ない場合はペナルティを強めて異常値を防ぐ
        model = Ridge(alpha=50.0) 
        model.fit(X, y)
        
        madori_results = dict(zip(X.columns, np.round(model.coef_, 0)))
        mean_area = df_m['専有面積_m2'].mean() if not df_m['専有面積_m2'].empty else 25.0
...
with tab2:
    if 'rules' not in st.session_state:
        ...
        # ⚠️ 修正ポイント：ベース単価が0より大きい（＝データが存在する）間取りをすべて取得
        valid_layouts = [k for k, v in bases.items() if v > 0]
        
        # もし一つも間取りが拾えなかった場合の保険（通常は起こらない）
        if not valid_layouts:
            st.error("アップロードされたデータから間取りが一つも抽出できませんでした。データ形式をご確認ください。")
        else:
